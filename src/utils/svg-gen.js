import { ParkMiller } from './rnd-park-miller';
import { sinCurves } from './sin';
...
export class Settings {
  constructor(width, height, points, lines, id, colors) {
    this.width = width;
    this.height = height;
    this.points = points;
    this.lines = lines;
    ...
    this.colors = [];
    this.colors.push(
      new GradientStop(0, colors[0]),
      new GradientStop(50, colors[1]),
      new GradientStop(100, colors[2])
    );
  }
  static default() {
    return new Settings(1200, 600, 10, 200, 0, [
      '#80c0f570',
      '#0471ca94',
      '#80c0f570',
    ]);
  ...
  svgD() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      if (i === 0) {
        ...
        svgP = ` C ${prevP.x + this.curve} ${prevP.y} ${p.x - this.curve} ${
          p.y
        } ${p.x} ${p.y}`;
      }
...
export function generate(settings) {
  ...
  for (let l of lines) {
    let d = l.svgD();
    let path = `<path style="stroke: url(#grad${settings.id}); fill:none;" d="${d}" ></path>`;
    paths += path;
  }
  ...
  let result = `
    ...
    <defs>
    <linearGradient id="grad${settings.id}" x1="0%" y1="0%" x2="100%" y2="0%">
    ${stops}
    </linearGradient>
    </defs>
    ${paths}
